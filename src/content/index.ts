...
import { AdView, SpinnerSuppressor, type OverlayAd } from "./overlay.js";
 ...
 * and answers with the ad to serve; we collapse the native loading
 * indicator and put the sponsored line in its place. Nothing on the page is
 * read beyond the busy/spinner selectors — prompts and conversations are
 * never touched.
 *
 * Two loops:
 * - accounting (1s): busy detection + impression ticks to the worker.
 * - placement (250ms): keeps the sponsored line glued to wherever the
 *   spinner is right now, so the swap is instant even as the site
 *   re-renders mid-stream.
 */
...
/** Placement loop: swap the native spinner for the sponsored line. */
